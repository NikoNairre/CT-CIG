...
class ControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.time_proj = Timesteps(128, True, downscale_freq_shift=0)   #no learnable parameters ,positional embedding，specifies the output channel to 128
        self.time_embedding = TimestepEmbedding(128, time_embed_dim)    #tiny learnable net

        self.embedding = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1),   #size scale to half
            nn.GroupNorm(2, 64),
            nn.ReLU(),
        ...
        for i in range(len(in_channels)):   #index in_channel[i] denotes number of dimensions
        ...
        self.mid_convs.append(nn.Sequential(
            nn.Conv2d(
                in_channels=out_channels[-1],   #256
    ...
    def forward(
        self,
        sample: torch.FloatTensor,              #controlnet_image
        ...
        t_emb = self.time_proj(timesteps)   #(b, 128)
        # `Timesteps` does not contain any weights and will always return f32 tensors
        # but time_embedding might actually be running in fp16. so we need to cast here.
        # there might be better ways to encapsulate this.
        t_emb = t_emb.to(dtype=sample.dtype)
        emb_batch = self.time_embedding(t_emb)  #(b, 256)

        # Repeat the embeddings num_video_frames times
        # emb: [batch, channels] -> [batch * frames, channels]
        emb = emb_batch
        sample = self.embedding(sample) #(b, 3, 512, 512) -> (b, 128, 256, 256)
        ...
        return {
            'out': sample,  #finally is 320 channels
            'scale': self.scale,
        }
