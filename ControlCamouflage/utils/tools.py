...
def get_pipeline(
    ...
    if os.path.isfile(pretrained_model_name_or_path):
        # load unet from local checkpoint, it works even with FIRM (because FIRM is added via monkey patch)
        ...
        firm_module = FrequencyInteractionAttentionModule(in_channels=320)
        unet.firm_module = firm_module
        ...
        # --- If you want to load pretrained model (VAE, SDXL) from HuggingFace，FIRM should also be considered ---
        # --- Only this way will weights match when loading unet weights with FIRM attached ---
        firm_module = FrequencyInteractionAttentionModule(in_channels=320)
        unet.firm_module = firm_module
    ...
    if unet_model_name_or_path is not None:
        ...
        pipeline.load_controlnext_unet_weights(  #this function ensure thats loading weights success even when saved unet only contains the trainable parameters
            unet_model_name_or_path,
